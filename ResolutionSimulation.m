...
% Defien Resolution Simulation Extent
min_pix = 10 ;
max_pix = 40;
num_pts = max_pix-min_pix+1;
pix_num = linspace(min_pix,max_pix,num_pts);

% Create HOlding Arrays
Nsurf = zeros(num_pts,1); 
Nline = zeros(num_pts,1); 
for pix_ind = 1:num_pts
   
    % Set Resolution Component Modes
    URresize = imresize(UR,[pix_num(pix_ind) pix_num(pix_ind)]);
    ULresize = imresize(UL,[pix_num(pix_ind) pix_num(pix_ind)]);
    
    % Compute Stokes Parameters
    [s0,s1,s2,s3] = sky.GetStokesRL(URresize,ULresize);
    ...
    pixcrop = pix_num(pix_ind)/2-1        ; % normally half the simulation space, but can be adjusted
    BeamSizeInPix = 2*w0/ (dx / pix_num(pix_ind)) ; % approximate size of the beam in pixels to ensure all of it is included in calculation
    thresh  = 0             ; % threshold for singularity exlcusion
    [Nline(pix_ind), ~, ~, ~,~] = sky.SkyNumLine(s1,s2,s3,s0,pixcrop,BeamSizeInPix, thresh) ;

    % Surface Integral Calculation
    [Nsurf(pix_ind), sigz] = sky.SkyNumSurf(s1,s2,s3);
    
    disp(pix_ind + ": " + Nsurf(pix_ind) + " , " + Nline(pix_ind))
